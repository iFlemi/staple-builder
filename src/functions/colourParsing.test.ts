...
import {stringToColourIdentity} from "@/functions/colourParsing";
...
describe('colourParsing tests', () => {
   ...
   it('should parse valid colours', () => {
      ...
      assertThat(result.isRight()).is(true)
      assertThat(result.getOrThrow()).is(match.array.unordered([Colour.White, Colour.Blue, Colour.Black, Colour.Green, Colour.Red]))
   })
   ...
   it('should fail with message when invalid input passed', () => {
      ...
      assertThat(result.getLeftOrElse(new Error('Should not get here')).message)
          .is(match.allOf([
